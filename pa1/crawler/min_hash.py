...
class MinHasher():
    ...
    def __init__(self, shingle_size = 4, hash_number=15):
    ...
    def min_hash(self, html):
        shingles = self.shingle_document(html)
        min_hash = []
    ...
    # ONLY FOR TESTING
    def jacard_similarity(self, html1, html2):
        shingles1 = set(self.shingle_document(html1, convert_to_int=False))
        shingles2 = set(self.shingle_document(html2, convert_to_int=False))
    ...
    def shingle_document(self, html, convert_to_int = True):
        ...
        tokens = list(filter(lambda x : len(x) > 1, tokens))    # remove either stopwords or stray utf-8 characters
